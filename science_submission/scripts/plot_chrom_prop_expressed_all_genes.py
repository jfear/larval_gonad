...
- y-axis: percent gene expressed (i.e. > 5) per chrom / number genes per arm * 100
...
def main():
    ...
    plt.rcParams["figure.figsize"] = (width, width * .8)

    fig, axes = plt.subplots(2, 2, sharex=True, sharey=True, gridspec_kw=dict(hspace=.05, wspace=0.05))
...
def plot_L3_scLineage(ax: plt.Axes, df: pd.DataFrame):
    ...
    ax.set_xlabel("")
    ax.set_title("L3 scRNA-Seq Lineages", y=.9)
    ax.legend(loc="upper left", bbox_to_anchor=[1, 1])
    return ax
