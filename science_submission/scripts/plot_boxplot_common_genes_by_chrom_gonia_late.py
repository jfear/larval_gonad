...
def plot_boxplot_common_genes_by_chrom_gonia_late(gs, box_kws=None, line_kws=None, stripplot_kws=None):
    box_kws = box_kws or {}
    ...
    germ_cells = []
    germ_cells.append(config['sel_cluster_order'][0])
    germ_cells.append(config['sel_cluster_order'][3])
    ...
    # Create axes
    fig = plt.gcf()
    gs0 = GridSpecFromSubplotSpec(1, 2, subplot_spec=gs, wspace=0.08)
...
if __name__ == '__main__':
    ...
    gs = GridSpec(1, 1)
    plot_boxplot_common_genes_by_chrom_gonia_late(gs[0, 0])
