...
male_bias_file_name = snakemake.input.male_bias
...
def main():
    ...
    ax.set_title('Male-Biased Expression Diversity')
    fig.savefig(oname, bbox_inches='tight')


def get_data():
    male_bias = open(male_bias_file_name).read().strip().split('\n')
    norm = pd.read_parquet(norm_file_name).reindex(male_bias).dropna()
    prop_on_per_cell = (
        (norm >= 1).mean()
        .rename('prop_gene_on')
        .to_frame()
