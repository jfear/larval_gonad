...
def get_data():
    ...
    y_genes = get_y_genes()
    zscores = zscores.reindex(y_genes).dropna()
    zscores.index = zscores.index.map(mapper)
    return zscores.reindex(sorted(zscores.index.tolist(), key=lambda x: x.lower()))


def get_y_genes():
    df = pd.read_parquet(fbgn2chrom)
    return df.query('chrom == "chrY"').index.tolist()
