---
title: Monocle2 Testis 3 Clustering
author: Justin Fear
...
markers <- fData(cds)[lit_genes$G,]$gene_short_name
plot_cell_clusters(cds, 1, 2, marker=markers, option = "B")
```

## Primary Spermatocytes
```{r fig.width=8, fig.height=12}
markers <- fData(cds)[lit_genes$EPS_MPS_LPS,]$gene_short_name
plot_cell_clusters(cds, 1, 2, marker=markers, option = "B")
```

## CySC
```{r fig.width=8, fig.height=12}
markers <- fData(cds)[lit_genes$cyst,]$gene_short_name
plot_cell_clusters(cds, 1, 2, marker=markers, option = "B")
```

## Terminal Epithelium
```{r fig.width=8, fig.height=10}
markers <- fData(cds)[lit_genes$T,]$gene_short_name
plot_cell_clusters(cds, 1, 2, marker=markers, option = "B")
```

## Pigment Cells
```{r fig.width=8, fig.height=6}
markers <- fData(cds)[lit_genes$P,]$gene_short_name
plot_cell_clusters(cds, 1, 2, marker=markers, option = "B")
```
