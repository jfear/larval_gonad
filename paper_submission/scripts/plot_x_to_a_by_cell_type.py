...
def main():
    df = pd.concat(
        [
            pd.read_feather(
                snakemake.input.tau, columns=["x_to_a_ratio", "cluster"]
            ).assign(gene_set="Tau"),
            pd.read_feather(
                snakemake.input.tsps, columns=["x_to_a_ratio", "cluster"]
            ).assign(gene_set="TSPS"),
            pd.read_feather(
                snakemake.input.widely_expressed, columns=["x_to_a_ratio", "cluster"]
