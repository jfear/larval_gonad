"""Calculate tau for each YOgn by sex.

I am calculating tau for each YOgn by sex separately. A score of 1 is highly
...
def main():
    ...
    # Calculate tau scores for males and females
    tau_scores = pd.concat(
        [
            norm_cnts["m"].apply(tau, axis=1).rename("male"),
            norm_cnts["f"].apply(tau, axis=1).rename("female"),
        ],
        axis=1,
        sort=True,
    )

    tau_scores.reset_index().to_feather(snakemake.output[0])
