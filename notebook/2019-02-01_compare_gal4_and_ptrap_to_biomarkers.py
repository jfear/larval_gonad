...
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8, 30), gridspec_kw=dict(width_ratios=[1, .05], wspace=.05))
sns.heatmap(_dat, yticklabels=True, cmap='viridis', ax=ax1, cbar_ax=ax2)
ax1.set_title(_title)
ax1.set_ylabel('Gene')

previous = ''
for i, gene in enumerate([l.get_text().replace('ptrap-', '').replace('scRNASeq-', '') for l in ax1.get_yticklabels()]):
    if gene == previous:
        continue
    ax1.axhline(i, color='w')
    previous = gene
    
ax1.axvline(4, color='w')
ax1.axvline(7, color='w')
ax1.axvline(8, color='w')
fig.savefig('../output/notebook/2019-02-01_ptrap_scrnaseq_heatmap.pdf', bbox_inches="tight")
