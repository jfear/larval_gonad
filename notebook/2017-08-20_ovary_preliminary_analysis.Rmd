...
mask <- rownames(fbgn.symbol) %in% rownames(ovary.data)
fbgns <- fbgn.symbol[mask, c("FBgn")]

# Subset ovary with only the FBgns we know
ovary.data = ovary.data[fbgns, ]

# Change rownames to use symbols
rownames(ovary.data) <- fbgn.symbol[fbgns, "symbol"]
```
...
```{r plots, fig.height=10, fig.width=15, warning=FALSE}
...
plt <- FeaturePlot(object=ovary, features.plot=genes, cols.use = c('grey', 'red'), pt.size=2, pch.use = 20, reduction.use='tsne')
```
