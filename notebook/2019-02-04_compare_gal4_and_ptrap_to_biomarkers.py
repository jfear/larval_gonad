...
for i, gene in enumerate([l.get_text().replace('ptrap-', '').replace('scRNASeq-', '') for l in ax1.get_yticklabels()]):
    if gene == previous:
        continue
    ax1.axhline(i, color='w', ls='--')
    previous = gene
    
ax1.axvline(4, color='w', ls='--')
ax1.axvline(7, color='w', ls='--')
ax1.axvline(8, color='w', ls='--')
fig.savefig('../output/notebook/2019-02-04_ptrap_scrnaseq_heatmap.pdf', bbox_inches="tight")
