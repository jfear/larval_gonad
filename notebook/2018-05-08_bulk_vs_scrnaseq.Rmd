---
title: Bulk vs scRNA-Seq
author: Justin Fear
date: 'Compiled: `r format(Sys.Date(), "%B %d, %Y")`'
---

This is a basic differential expression analysis comparing Bulk RNA-Seq vs
single cell RNA-Seq. For this comparison I have selected two different bulk
RNA-Seq data sets {TCP and TDT}.

* TCP: Testis cleaned using papain. We expect this to be closest to wildtype
  because its treatment is the mildest.
* TDT: Testis dissociated using tripsin. We expect this to be closest to the
  scRNA-seq because it is the most similar treatment.
...
OUTPUT = file.path('../output/bulk_vs_scrnaseq')
...
sample.table.filename = '../config/bulk_vs_scrnaseq_sampletable.tsv'
colData <- read.table(sample.table.filename, sep='\t', header=TRUE, stringsAsFactors=F)

# Assumption: data directory and extension to featurecounts output
mk.featurecounts <- function(x){
	fname <- file.path('..', 'bulk-rnaseq-wf', 'data', 'rnaseq_samples', x, paste0(x, '.cutadapt.bam.featurecounts.txt'))
	dat <- read.table(fname, stringsAsFactors=F, skip=2)[,c('V1', 'V7')]
	colnames(dat) <- c('FBgn', x)
	return(dat)
}


bulk <-  merge(mk.featurecounts('C1_TDT'), mk.featurecounts('C2_TDT'), by='FBgn')
bulk <-  merge(bulk, mk.featurecounts('C3_TDT'), by='FBgn')
bulk <-  merge(bulk, mk.featurecounts('C4_TDT'), by='FBgn')
bulk <-  merge(bulk, mk.featurecounts('B5_TCP'), by='FBgn')
bulk <-  merge(bulk, mk.featurecounts('B6_TCP'), by='FBgn')
bulk <-  merge(bulk, mk.featurecounts('B7_TCP'), by='FBgn')
bulk <-  merge(bulk, mk.featurecounts('B8_TCP'), by='FBgn')


# Import scRNA-seq
scRNAseq <- read.table('../output/combined_testis_force/raw_agg.tsv', header=T)

# Merge everything
dat <- merge(scRNAseq, bulk, by='FBgn', all.x=T, all.y=F)
rownames(dat) <- dat$FBgn
dat <- dat[, !names(dat) %in% 'FBgn']
dat <- dat[, colData$samplename]
...
dds <- DESeqDataSetFromMatrix(countData=dat, colData=colData, design=~group, ignoreRank=TRUE)
```
...
res.list <- list(
                 tcp=results(dds, contrast=c('group', 'TCP', 'sc'), alpha=0.01),
                 tdt=results(dds, contrast=c('group', 'TDT', 'sc'), alpha=0.01)
                 )
res.list.lookup <- list(
			tcp="TCP vs scRNA-Seq",
			tdt="TDT vs scRNA-Seq"
			)
```
...
for (name in names(res.list)){
  mdcat('## ', res.list.lookup[[name]])
  fn <- file.path(OUTPUT, paste0(name, '.tsv'))
  ...
  for (sel in names(sel.list[[name]])){
    fn <- file.path(OUTPUT, paste0(name, '.', sel, '.tsv'))
...
write.table(full.enrich.table, file=file.path(OUTPUT, 'functional_enrichment.tsv'), row.names=FALSE, sep='\t')
```
