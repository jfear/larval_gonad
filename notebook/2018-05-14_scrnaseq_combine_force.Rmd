...
```{r}
skip = c()
for (j in unique(combined@ident)){
  if (j %in% skip){
    next()
  }
  print(j)
...
```{r}
#' Wrapper for Seurat::FindConservedMarkers to run each marker and make a
#' single dataframe.
#'
#' @param object Seurat object
#' @param ... other parameters to passed to FindConservedMarkers
#'
FindAllConservedMarkers <- function(object, ...){
  marker.list <- list()
  for (j in unique(object@ident)){

    marker <- tryCatch(
      {
        FindConservedMarkers(
          object = object,
          ident.1 = as.integer(j),
          grouping.var = 'rep',
          only.pos = TRUE,
          print.bar = FALSE,
          ...
    ...
    if (is.null(marker)) next()

    marker$cluster <- as.integer(j)
    marker$gene <- rownames(marker)
    marker.list[[j]] <- marker
  }

  markers.df <- rbindlist(marker.list)
  return(markers.df)
}
```
...
for (i in c(0.4, 1.0)) {
  ...
  # Get conserved markers by looking at each rep.
  cons.markers <- FindAllConservedMarkers(object = combined)
  cons.markers <- merge(
    fbgn2symbol,
    cons.markers,
    by.x='primary_FBgn',
  ...
  fname.cons <- paste0('biomarkers_conserved_', name, '.tsv')
  save_biomarkers(markers = cons.markers, dir = OUTDIR, fname = fname.cons)
