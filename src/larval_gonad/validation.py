...
class GeneValidator(object):
    ...
    def lvl3_protein(self):
        ...
        if (
            ...
            if self.proceeds(self.lit_gene, self.biomarker, self.germ_lineage):
                self.score = 2
                return True
        elif (
            self.lit_gene.issubset(self.cyst_lineage)
            ...
            if self.proceeds(self.lit_gene, self.biomarker, self.cyst_lineage):
                self.score = 2
                return True
    ...
    def lvl4_protein(self):
        ...
        if (
            ...
            if self.proceeds(self.lit_gene, self.zscore, self.germ_lineage):
                self.score = 1
                return True
        elif (
            self.lit_gene.issubset(self.cyst_lineage)
            ...
            if self.proceeds(self.lit_gene, self.zscore, self.cyst_lineage):
                self.score = 1
                return True

    def proceeds(self, protein_cell_types, cell_types, lineage):
        """Check if any cell types proceed the protein index."""
        for protein, rna in product(protein_cell_types, cell_types):
