...
def _add_legend(ax):
    off = mpatches.Patch(color="#450457", label="not expressed")
    low = mpatches.Patch(color="#ff7800", label="low expression")
    high = mpatches.Patch(color="#f8e621", label="high expression")
    none = mpatches.Patch(color="#d3d3d3", label="not analyzed")
    ax.legend(loc="upper left", bbox_to_anchor=[1, 1], handles=[off, low, high, none])
    return ax
