...
#gonia <- row.names(subset(fData(dds_germ), gene_short_name %in% c('peb', 'Phf7', 'p53', 'nos', 'bgcn')))
#early <- row.names(subset(fData(dds_germ), gene_short_name %in% c('vas', 'bam', 'tut', 'tej', 'mle')))
#late <- row.names(subset(fData(dds_germ), gene_short_name %in% c('c-cup', 'soti', 'sowi', 'can', 'nht', 'dj', 'ocn', 'fzo')))

cth <- newCellTypeHierarchy()

# Spermatogonia
peb <- row.names(subset(fData(dds_germ), gene_short_name  == 'peb'))
Phf7 <- row.names(subset(fData(dds_germ), gene_short_name  == 'Phf7'))
p53 <- row.names(subset(fData(dds_germ), gene_short_name  == 'p53'))
nos <- row.names(subset(fData(dds_germ), gene_short_name  == 'nos'))
bgcn <- row.names(subset(fData(dds_germ), gene_short_name  == 'bgcn'))
cth <- addCellType(cth, "Gonia", classify_fun = function(x) {
			   x[peb,] > 0 & x[Phf7,] > 0 & x[p53,] > 0 & x[nos,] > 0 & x[bgcn,] > 0
		    })


# Early Spermatocytes
vas <- row.names(subset(fData(dds_germ), gene_short_name  == 'vas'))
bam <- row.names(subset(fData(dds_germ), gene_short_name  == 'bam'))
tut <- row.names(subset(fData(dds_germ), gene_short_name  == 'tut'))
tej <- row.names(subset(fData(dds_germ), gene_short_name  == 'tej'))
mle <- row.names(subset(fData(dds_germ), gene_short_name  == 'mle'))

cth <- addCellType(cth, "Early", classify_fun = function(x) {
			   x[vas,] > 0 & x[bam,] > 0 & x[tut,] > 0 & x[tej,] > 0 & x[mle,] > 0
		    })
...
# Late Spermatocytes
ccup <- row.names(subset(fData(dds_germ), gene_short_name  == 'c-cup'))
soti <- row.names(subset(fData(dds_germ), gene_short_name  == 'soti'))
sowi <- row.names(subset(fData(dds_germ), gene_short_name  == 'sowi'))
can <- row.names(subset(fData(dds_germ), gene_short_name  == 'can'))
nht <- row.names(subset(fData(dds_germ), gene_short_name  == 'nht'))
dj <- row.names(subset(fData(dds_germ), gene_short_name  == 'dj'))
ocn <- row.names(subset(fData(dds_germ), gene_short_name  == 'ocn'))
fzo <- row.names(subset(fData(dds_germ), gene_short_name  == 'fzo'))
cth <- addCellType(cth, "Late", classify_fun = function(x) {
			   x[ccup,] > 0 & x[soti,] > 0 & x[sowi,] > 0 & x[can,] > 0 & x[nht,] > 0 & x[dj,] > 0 & x[ocn,] > 0 & x[fzo,] > 0
		    })
```
