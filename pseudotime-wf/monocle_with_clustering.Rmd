...
png('../output/pseudotime-wf/recluster.png', width = 1600, height = 1600, res = 150)
...
```{r plot.trajectory}
png('../output/pseudotime-wf/reclustered_trajectory.png', width = 1600, height = 1600, res = 150)
plot_cell_trajectory(dds_order, color_by = 'res.0.6')
dev.off()
```

```{r output}
save(dds_order, file = '../output/pseudotime-wf/monocle_recluster.Robj')
#load('../output/pseudotime-wf/monocle_recluster.Robj')
```
...
write.table(cell.loc, file = '../output/pseudotime-wf/cell_location_recluster.tsv', sep = '\t', quote = F, row.names = F)
...
write.table(tree.loc, file = '../output/pseudotime-wf/tree_location_recluster.tsv', sep = '\t', quote = F, row.names = F)

branch.point <- dds_order@auxOrderingData[['DDRTree']]$branch_points
write(branch.point, file = '../output/pseudotime-wf/branch_points_recluster.txt', sep = '\n')
...
write.table(metadata, file = '../output/pseudotime-wf/pseudotime_recluster.tsv', sep = '\t', quote = F, row.names = F)
```
