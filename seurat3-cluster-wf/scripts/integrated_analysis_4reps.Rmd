...
knitr::opts_chunk$set(
    ...
    echo = FALSE,
    cache.lazy = FALSE,
    fig.path = "integrated_4reps_figures/",
    fig.ext = c("png", "svg"),
    fig.width = 8
)
```
...
```{r dimplots, fig.width=8, fig.height=8}
...
combined <- RunPCA(combined, npcs = 30, features = VariableFeatures(combined), verbose = FALSE)
combined <- RunUMAP(combined, reduction = "pca", dims = PCA_DIMS)
```

```{r find_clusters}
combined <- FindNeighbors(combined, reduction = "pca", dims = PCA_DIMS, verbose = FALSE)
...
DimPlot(combined, reduction = "umap", split.by = "sample", label = TRUE)
```
...
FeaturePlot(combined, features = lg, ncol = 2, pt.size = .4, min.cutoff = 0, order=TRUE)
```

```{r vas_bam, fig.width=8, fig.height=4}
FeaturePlot(combined, features = c("vas", "bam"), pt.size = .4, min.cutoff = 0, order=TRUE)
```
...
FeaturePlot(combined, features = c("aly", "can", "nht", "soti"), ncol = 2, pt.size = .4, min.cutoff = 0, order=TRUE)
```
...
FeaturePlot(combined, features = lg, pt.size = .4, min.cutoff = 0, order=TRUE)
```
...
write.table(
    biomarkers,
    file = file.path(DATA_DIR, paste("combined", paste0("n", length(SAMPLES)), "biomarkers.tsv", sep="_")),
    sep = "\t",
    quote = FALSE,
    row.names = FALSE,
    col.names = TRUE
...
feather::write_feather(
    biomarkers,
    path = file.path(DATA_DIR, paste("combined", paste0("n", length(SAMPLES)), "biomarkers.feather", sep="_"))
)
```
...
save(
    tesits1 = combined,
    file = file.path(DATA_DIR, paste0(paste("combined", paste0("n", length(SAMPLES)), sep="_"), ".Robj"))
)
```
...
df$cluster <- df[, SEL_RESOLUTION] %>% pull(.)

write.table(
    df,
    file = file.path(DATA_DIR, paste("combined", paste0("n", length(SAMPLES)), "clusters.tsv", sep="_")),
    sep = "\t",
    quote = FALSE,
    row.names = FALSE,
    col.names = TRUE
...
feather::write_feather(
    df,
    path = file.path(DATA_DIR, paste("combined", paste0("n", length(SAMPLES)), "clusters.feather", sep="_"))
)
```
