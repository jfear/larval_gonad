...
hub.cache <- '../../lcdb-wf/include/AnnotationHubCache'
...
featurecounts.strandedness <- 's2'  # unstranded
...
colData$salmon.path <- sapply(
    colData$samplename,
    function (x) file.path('..', 'output', 'bulk2-rnaseq-wf', 'rnaseq_samples', x, paste0(x, '.salmon'), 'quant.sf')
)
...
res.list[['all']] <- list(
    res=lfcShrink(dds, contrast=c('group', 'TCP', 'OCP')),
    dds=dds,
