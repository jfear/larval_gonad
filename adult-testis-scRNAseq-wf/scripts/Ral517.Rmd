---
title: Seurat3 Ral517 Clustering
author: Justin Fear
...
SEL_RESOLUTION <- "RNA_snn_res.0.3"
PCA_DIMS <- 1:9

PROJ_DIR <- "/home/fearjm/Projects/larval_gonad"
DATA_DIR <- file.path(PROJ_DIR, paste0("output/adult-testis-scRNAseq-wf/", SAMPLE, "/outs/filtered_feature_bc_matrix/"))
OUTDIR <- file.path(PROJ_DIR, "output/adult-testis-scRNAseq-wf")
...
VizDimLoadings(sobj, dims = 1:8, reduction = "pca", ncol = 2)
```
...
lg <- fbgn2symbol %>%
    filter(FBgn %in% c(lit_genes[["SP"]], lit_genes[["SP_ES"]])) %>%
    pull("FBgn")
...
lg <- fbgn2symbol %>%
    filter(FBgn %in% lit_genes[["SP_ES_MS_LS"]]) %>%
    pull("gene_symbol")
...
lg <- fbgn2symbol %>%
    filter(FBgn %in% c(lit_genes[["EC"]], lit_genes[["CY"]])) %>%
    pull("gene_symbol")
